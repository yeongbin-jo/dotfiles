...
present: an Artifact URL, the authoring Claude chat, an HTML draft, a named previous version,
or an instruction such as “keep the architecture/content and improve only the visualization.”

A supplied URL proves the referenced Artifact exists but may not expose its authoring conversation
or visibility. First inspect the URL as a baseline, then locate the authenticated authoring chat
when possible. Do not silently
...
the request already authorizes a new Artifact.

## Preserve identity and intent

For an update:

1. Capture the existing Artifact URL, title, major sections, interactions, visibility when known,
   and representative
...
4. After saving, read the original Artifact again. Record whether it now serves the revision; if
   Claude issued a replacement, return the old→new mapping. Recheck an unauthenticated public URL
   only when public sharing was explicitly requested.
...
Never start parallel candidates and pick the prettiest result: that creates duplicate Artifact state,
