...
되살리는데 거기 claude 가 없다 — 그래서 **창은 살아있고 claude 만 죽어 맨 셸이 남는다.**
이 스킬이 그 빈칸만 정확히 메운다.
...
`--window N` 으로 한 창만 복구. 원장 경로는 `NYDUS_LEDGER` 로 교체 가능(테스트용).
...
1. **argv** — `--resume/-r <id>` 또는 `--session-id <id>` 가 커맨드라인에 있으면 확정.
   한 번 이 스킬로 복구된 창은 이후 영원히 argv 에 ID 를 달고 있다(자기 식별).
...
- **살아있는 claude 는 절대 건드리지 않는다** — 죽은 창만 대상.
